...
Cli.run = function run(processArgv) {
  try {
    ...
    if (oldConfig) {
      console.warn('WARN: ionic.config.js is deprecated and no longer used for v2 projects\n');
    }
    ...
    if (gulpFound) {
      ...
      return Q.nfcall(gulp.start.bind(gulp), beforeHook).then(
        ...
        function(e){ //task error, let gulp handle it
          var taskName = taskSetting.name;
          if (/serve|build|run|emulate|upload/.test(taskName)) {
            console.warn('WARN: No \'' + beforeHook + '\' gulp task found!');
            if (oldConfig && gulp.tasks.build) {
              console.info('Your gulpfile contains a \'build\' task already! Add:\n')
              console.log('    gulp.task(\'' + taskName + ':before\', [\'build\']);\n');
              console.info('to your gulpfile to have Ionic CLI run build before ' + taskName);
            } else {
              console.warn('If your app requires a build step, make sure it runs before ' + taskName + ';\n.');
            }
            console.log();
          }
          // Only some commands return promises
      ...
      console.warn('If your app requires a build step, make sure it runs before ' + taskSetting.name + '\n');
      return Q.fcall(taskInstance.run.bind(taskInstance), Cli, argv);
    }
