...
Cli.run = function run(processArgv) {
  try {
    ...
    if (!task) {
      return helpUtil.printTaskListShortUsage(Cli.getAllTaskSettings(), taskName, Cli.VERSION);
    }
...
Cli.getTaskSettingsByName = function getTaskSettingsByName(taskName) {
  ...
  Object.keys(TASKS).every(function(listName) {
    if (listName === taskName) {
      task = require(TASKS[listName]);
      return false;
    }
    return true;
  });
...
Cli.getAllTaskSettings = function getAllTaskSettings() {
  return Object.keys(TASKS).map(function(listName) {
    return require(TASKS[listName]);
  });
};
