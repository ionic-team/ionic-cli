...
var IonicStats = require('./ionic/stats').IonicStats;
var IonicStore = require('./ionic/store');
var IonicConfig = new IonicStore('ionic.config');
var Info = IonicAppLib.info;
var IonicProject = IonicAppLib.project;
var Ionitron = require('./ionic/ionitron');
var optimist = require('optimist');
var path = require('path');
var fs = require('fs');
var settings = require('../package.json');
var Tasks = require('./tasks/cliTasks');
...
Cli.run = function run(processArgv) {
  try {
     ...
     */
    if (argv.help || argv.h) {
      return Q.fcall(Cli.printHelpLines);
    }
    ...
    var taskSetting = Cli.getTaskByName(taskName);

    // If no task is found then just provide the list of tasks available
    if (!taskSetting) {
      return Q.fcall(Cli.printAvailableTasks, taskName);
    }

    log.debug('Task setting:', taskSetting);

    var booleanOptions = Cli.getListOfBooleanOptions(taskSetting.options);

    // Remove all possible task boolean options from the args
    argv = optimist(processArgv.slice(2))
      .boolean(booleanOptions)
      .argv;

    // Get the task 'class'
    var taskInstance = Cli.lookupTask(taskSetting.module);
 ...
 */
Cli.getTaskByName = function getTaskByName(taskName) {
  return TASKS.find(function(task) {
    return (task.name === taskName) || (Array.isArray(task.alt) && task.alt.indexOf(taskName) !== -1);
  });
};
...
Cli.printIonic = function printIonic() {
  return [
    ...
    ' | |/ _ \\| \'_ \\| |/ __| ',
    ' | | (_) | | | | | (__     ',
    ' |_|\\___/|_| |_|_|\\___|  CLI v' + settings.version
  ];
};

function printTemplate(lineArray) {
  console.log(lineArray.reduce(function(all, line) {
    all += (line !== null) ? line + '\n' : '';
    return all;
  }, ''));
}
 ...
 */
Cli.printAvailableTasks = function printAvailableTasks(taskName) {
  var taskLines = TASKS
