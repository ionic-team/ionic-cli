...
Cli.run = function run(argv) {
  ...
  try {    
    ...
    // console.log('taskinstance run with argv', argv);
    var promise = taskInstance.run(Cli, argv);
    // console.log('run is going');
    return promise;
  } catch (ex) {
...
Cli.reportExtras = function reportExtras(err) {
  ...
  if (Cli.dev()) {
    console.log('An error occured and was reported to Opbeat', err);
...
Cli.handleUncaughtExceptions = function handleUncaughtExceptions(err, url) {
  console.log('An uncaught exception occured and has been reported to Ionic'.error.bold);
