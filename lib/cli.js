...
Cli.runWithGulp = function runWithGulp(argv, taskInstance) {
  ...
  // run beforeHook
  return Q.nfcall(gulp.start.bind(gulp), beforeHook)
  // run cmd
    .then(function() {
      return Q.fcall(taskInstance.run.bind(taskInstance), Cli, argv);
    })
    // task error, let gulp handle it
    .catch(function(e) {
      //TODO check if task exists before running gulp unnecessarily
      // if there is no hook task, but it's a v2 app and a command that usually needs a hook
      if (e.missingTask && Cli.isBuildCommand(cmdName) && argv.v2) {
  ...
  // run afterHook
    ...
    .catch(function(e){
      // TODO this should be handled appropriately, but for now at least
      // pass it along
      throw e;
    });
}
