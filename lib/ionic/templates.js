...
IonicTask.prototype.fetchStarterTemplates = function() {
  ...
  //TODO: change this to master when merged in.
  var downloadUrl = 'https://raw.githubusercontent.com/driftyco/ionic-cli/starter-templates-listing/lib/ionic/starter-templates.json';
  var starterTemplateJsonPath = path.join('./', 'starter-templates.json');
  ...
  request({ url: downloadUrl, proxy: proxy }, function(err, res, html) {
    if(!err && res && res.statusCode === 200) {
      ...
      var templatesJson = require(starterTemplateJsonPath);
...
IonicTask.prototype.run = function(ionic) {
  ...
  .then(function(starterTemplates) {
    console.log('fetch', starterTemplates);

    self.list(starterTemplates);
  })
