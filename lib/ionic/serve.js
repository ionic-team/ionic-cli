...
function runAppScriptsServer(argv) {
  ...
  options = npmScripts.consolidateOptions(['livereloadPort', 'r'], options);
  ...
  options.livereloadPort = options.livereloadPort || serveUtils.DEFAULT_LIVE_RELOAD_PORT;
  ...
  return serveUtils.findClosestOpenPort(options.address, options.port).then(function(portFound) {
    ...
    return serveUtils.findClosestOpenPort(options.address, options.livereloadPort);
  })
  .then(function(portFound) {
    options.livereloadPort = portFound;
