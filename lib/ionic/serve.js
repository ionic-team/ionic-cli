...
IonicTask.prototype.checkPorts = function(isIonicServer, testPort) {
  var q = Q.defer();
...
IonicTask.prototype.showFinishedServeMessage = function() {
  ...
  if(this.launchLab || this.launchBrowser) {
    ...
    // console.log('Launching - openUrl', openUrl, 'and browser:', this.browser);
    try {
  ...
  // if(this.launchLab) {
  //   var open = require('open');
  //   open(this.host(this.port) + IONIC_LAB_URL);
  //   // open(this.host(this.port), this.browser);
  // } else if (this.launchBrowser) {
  //   var open = require('open');
