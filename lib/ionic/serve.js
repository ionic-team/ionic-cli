...
IonicTask.prototype.loadSettings = function(cb) {
  ...
  process.stdin.on('readable', function() {
    ...
    if(input.toLowerCase() == 'restart' || input.toLowerCase() == 'r') {
      self._goToUrl('/?restart=' + Math.floor((Math.random() * 899999) + 100000));

    } else if(input.toLowerCase().indexOf('goto ') === 0 || input.toLowerCase().indexOf('g ') === 0) {
      var url = input.replace('goto ', '').replace('g ', '');
      self._goToUrl(url);

    } else if(input.toLowerCase() == 'consolelogs' || input.toLowerCase() == 'c') {
      self.printConsoleLogs = !self.printConsoleLogs;
      console.log('Console log output: '.green + (self.printConsoleLogs ? 'enabled' : 'disabled'));

    } else if(input.toLowerCase() == 'serverlogs' || input.toLowerCase() == 's') {
    ...
    } else if(/exit|quit|close|stop/i.test(input)) {
      process.exit();
    } else {
      console.log('Invalid server command'.error.bold);
...
IonicTask.prototype.printCommandTips = function(ionic) {
  console.log('Server commands, enter:'.green.bold);
  console.log(' restart' + ' or '.green + 'r' + ' to restart the app from the root'.green);
  ...
  console.log(' exit' + ' to shutdown the server and exit'.green);
  console.log('');
};
  ...
  try {
    ...
    if(addresses.length === 1) {
      ...
      for(var x=0; x<addresses.length; x++) {
        console.log( (' ' + (x+1) + ') ' + addresses[x].ip + ( addresses[x].dev ? ' (' + addresses[x].dev + ')' : '' )).yellow );
      }
      ...
      prompt.get({properties: promptProperties}, function (err, promptResult) {
        ...
        self.ionic.fail('Invalid IP address selection');
      });
