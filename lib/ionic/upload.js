...
function addCachebusters(indexPath) {
  ...
  $('script').each(function(i, el){
    el.attribs.src += "?ionicCachebuster=" + randomString;
  });
  $('link').each(function(i, el){
    el.attribs.href += "?ionicCachebuster=" + randomString;
  });
  try {
...
function removeCachebusters(indexPath) {
  ...
  var $ = cheerio.load(indexHtml);
  $('script').each(function(i, el){
    el.attribs.src = el.attribs.src.substring(0, el.attribs.src.indexOf("?ionicCache"));
  });
  $('link').each(function(i, el){
    el.attribs.href = el.attribs.href.substring(0, el.attribs.href.indexOf("?ionicCache"));
  });
  try {
    fs.writeFileSync(indexPath, $.html());
  } catch(e) {
