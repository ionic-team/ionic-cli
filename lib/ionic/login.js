...
IonicTask.prototype.requestLogIn = function(ionic, callback, saveCookies) {
  ...
  function (err, response, body) {
    ...
    if(saveCookies) {
      ...
      transformedCookies = self.cookieData.get(ionic.IONIC_DASH);
    } else {
      try {
        var jsonString = JSON.stringify(jar.getCookies(ionic.IONIC_DASH, null, 2));
