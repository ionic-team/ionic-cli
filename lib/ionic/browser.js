...
var crosswalkEngineVersion = 'c0.6.1',
    cordovaAndroidVersion = 'c0.5.5',
...
IonicTask.prototype.addCordova40xProject = function addCordova40xProject() {
  var command =  ['cordova platform add ./engine/cordova-android-', cordovaAndroidVersion, '/'].join('');
...
IonicTask.prototype.removeOldCrosswalkEngine = function removeOldCrosswalkEngine() {
  var rmCmd = 'cordova plugin rm org.apache.cordova.engine.crosswalk';
  var result = shelljs.exec(rmCmd);
}
...
IonicTask.prototype.upgradeCrosswalk = function upgradeCrosswalk(addPlugin) {

  //Remove the old engine, download the latest, and add it.
  if(typeof addPlugin == 'undefined') {
    addPlugin = false;
  }

  console.log('Updating your Ionic project with the latest build of Crosswalk'.green)
  //Old plugin name: org.apache.cordova.engine.crosswalk
  //New plugin name: org.crosswalk.engine
  var self = this;

  self.removeOldCrosswalkEngine();

  self.getCordovaCrosswalkEngine()
  .then(function success(data) {
    console.log('Got cordova crosswalk engine: ', data);
    if(addPlugin) {
      self.addCrosswalkPlugin();
    }
    console.log('Done')
  }, function failed(error){ 
    self.ionic.fail('Failed retrieving the Crosswalk engine plugin');
  })
}
...
IonicTask.prototype.addBrowser = function addBrowser() {
  ...
  if(browserToInstall.indexOf('@') !== -1) {
    ...
    browserVersion = crosswalkVersions[2].version;
  }
...
IonicTask.prototype.run = function run(ionic) {
  ...
  var validCommands = 'add remove rm list ls revert versions upgrade'.split(' ');
