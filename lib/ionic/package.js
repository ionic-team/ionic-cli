    ...
    login.get(ionic, function(jar) {
      ...
      for(var i = 0; i < platforms.length; i++) {
        ...
        prompt.get({properties: properties}, function (err, result) {
          ...
          for (var property in properties) {
            if(result[property] && !project.get(property)) {
              project.set(property, result[property]);
            }
          }
          project.save();
          ...
          for (var property in project) {
            ...
            if(property.indexOf(platform) == 0) {
              form.append(property, project.get(property));
            }
          }
          ...
          switch(platform) {
            case 'android':
              ...
              break;
            case 'ios': 
              var certificateFilePath = path.resolve(project.get('ios_certificate'));
              ...
              var profileFilePath = path.resolve(project.get('ios_profile'));
              if(fs.existsSync(profileFilePath)) {
                form.append('ios_profile_file', fs.createReadStream(profileFilePath));
              } else {
                project.remove('ios_profile');
                project.save();
