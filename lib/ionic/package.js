    ...
    login.get(ionic, function(jar) {
      ...
      for(var i = 0; i < platforms.length; i++) {
        ...
        prompt.get({properties: properties}, function (err, result) {
          ...
          // Add the project properties to the post
          for (var property in project) {
            if(property.indexOf(platform)) {
              form.append(property, project[project]);
            }
          }
          
          // Gimmie those sweet sweet files
          switch(platform) {
            case 'android':
              var keystoreFilePath = path.resolve(project.android_keystore);
              if(mode = 'release') {
                if(fs.existsSync(keystoreFilePath)) {
                  form.append('android_keystore_file', fs.createReadStream(keystoreFilePath));
                } else {
                  delete project.android_keystore;
                  ionic.saveConfig(project);
          ...
          var url = ionic.IONIC_DASH+'export/'+project.app_id;
