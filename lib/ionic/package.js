        ...
        if (platform === 'android') {
          if (buildMode === 'debug') {
            ...
            prompt.get([
              ...
              if (err) {
                return q.reject('Error retrieving Android information: ' + err);
              }
          ...
          prompt.get([
              { name: 'cert', description: 'Certificate File:'.yellow.bold, required: true },
              { name: 'cert-password', description: 'Certificate Password:'.yellow.bold, hidden: true, required: true },
              { name: 'provisioning-profile', description: 'Provisioning Profile:'.yellow.bold, required: true }
          ], function(err, result) {
            if (err) {
              return q.reject('Error retrieving iOS information: ' + err);
            }
