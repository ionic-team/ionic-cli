...
IonicTask.prototype.getCordovaInfo = function(info) {
  ...
  info.cordova = 'Cordova CLI ' + result.output.replace('\n', '');
}

IonicTask.prototype.getXcodeInformation = function() {
...
IonicTask.prototype.getIonicVersions = function(info) {
  ...
  var ionicVersion = require( path.resolve('www/lib/ionic/version.json') ).version;

  info.ionic_cli = ionicCliVersion;
  info.ionic = ionicVersion;

  // console.log('Ionic CLI version', ionicCliVersion)
...
IonicTask.prototype.getOsEnvironment = function(info) {
  ...
  switch(os.type()) {
    case 'Darwin':
      ...
      info.xcode = self.getXcodeInformation();
      break;
    case 'Windows_NT':
