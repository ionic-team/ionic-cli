...
function run(ionic, argv) {
  try {
    ...
    var name = extractName(argv._[2]); // TODO support multiple names
    ...
    var generatorOptions = {
      ...
      suppliedName: name.file,
      includeSpec: includeSpec,
      includeSass: includeSass
    };

    var componentsDir = path.join(process.cwd(), 'src', 'components', name.path);
    var directivesDir = path.join(process.cwd(), 'src', 'components', name.path);
    var pagesDir = path.join(process.cwd(), 'src', 'pages', name.path);
    var pipesDir = path.join(process.cwd(), 'src', 'pipes', name.path);
    var providersDir = path.join(process.cwd(), 'src', 'providers', name.path);
