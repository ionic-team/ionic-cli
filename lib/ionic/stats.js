...
function create_client(token, config) {
  ...
  metrics.send_request = function(endpoint, data, callback) {
    ...
    if (endpoint === '/import') {
      ...
      if (!key) {
        throw new Error('The Mixpanel Client needs a Mixpanel api key when importing old events: ' +
                        ' `init(token, { key: ... })`');
      }
...
exports.IonicStats = {
  t: function(additionalData) {
    try {
      ...
      var packageJson = exports.getVersion();
      ...
      var argsWhitelist = 'add remove list update check debug release search --livereload ' +
        '--consolelogs --serverlogs --no-cordova --nobrowser --nolivereload --noproxy --no-email ' +
        '--debug --release --device --emulator --sass --splash --icon --v2 --ts'.split(' ');
      ...
      try {
        statsData.cli_version = packageJson.version;
      } catch (e2) {
        throw e2;
      }
      ...
      this.mp(cmdName, _.extend(statsData, {
        os: info.os,
        gulp: info.gulp,
        node: info.node
      }));
