...
IonicStartTask.prototype._printUsage = function() {
  ...
  process.stderr.write('   -t, --template <STARTER TEMPLATE> ...... starter template to use (tabs, sidemenu, blank) or github user/repo of template (username/my-template-repo)\n');
...
IonicStartTask.prototype.run = function(ionic) {
  ...
  var starterProject = (argv.template || argv.t || argv._[2] || 'tabs').toLowerCase();
...
IonicStartTask.prototype._getRepoDetails = function (projectType) {
  // Repo details
  var repo = {
    name: 'ionic-starter-' + projectType,
    url: 'https://github.com/'
  };

  var project = projectType.split('/');
  if (project.length > 1) {
    repo.name = project[1];
    repo.url += project[0] + '/' + project[1] + '/archive/master.zip';
  } else {
    repo.url += 'driftyco/ionic-starter-' + project[0] + '/archive/master.zip' ;
  }
  return repo;
}
...
IonicStartTask.prototype._fetchAndWriteSeed = function(projectType) {
  ...
  wrapperPromise.then(function() {
    // Get the starter project repo details
    var repo = self._getRepoDetails(projectType);

    Ionic.fetchRepo(self.targetPath, repo.name, repo.url).then(function(repoFolderName) {
      var npmQ = Q.defer();
      var gulpQ = Q.defer();
      var pluginQ = Q.defer();
      var q = Q.defer();
      ...
      pluginQ.promise.then(function() {
        ...
        ionicBower.setAppName(self.appName);

        try {
          fs.unlinkSync(self.targetPath + '/www/README.md');
        } catch(e) {}

        if(argv.sass) {
          console.log('setup sass'.green.bold);
          var setupTask = new setup.IonicSetupTask();
          q = setupTask.sassSetup(self.ionic)
        } else {
          q.resolve();
        }
      ...
      console.error('Valid project types are blank, tabs, or sidemenu, or a github url to a starter project (or see more on our starter page: http://ionicframework.com/getting-started/)'.error.bold);
