...
IonicStartTask.prototype.run = function(ionic) {
  ...
  if( fs.existsSync(this.targetPath) ) {
    ...
    if( fs.readdirSync(this.targetPath).length ) {
      // only freak if target path already has files in it
      return this.ionic.fail('The directory '.error.bold + this.targetPath + ' already exists, please remove it if you would like to create a new ionic project there.'.error.bold);
    }
  } else {
...
IonicStartTask.prototype.fetchCodepen = function() {
  // Edit Url:  http://codepen.io/ionic/pen/gblny
  // Download:  http://codepen.io/ionic/share/zip/gblny/
  var self = this;
  var splt = self.template.split('?')[0].split('#')[0].split('/');
  var wwwPath = path.join(self.targetPath, 'www');
  var codepenId = splt[ splt.length - 1 ];
  if(!codepenId) {
    codepenId = splt[ splt.length - 2 ];
  }
  ...
  var htmlUrl = 'http://codepen.io/ionic/pen/' + codepenId + '.html';
  var cssUrl = 'http://codepen.io/ionic/pen/' + codepenId + '.css';
  var jsUrl = 'http://codepen.io/ionic/pen/' + codepenId + '.js';

  var proxy = process.env.PROXY || null;

  request({ url: htmlUrl, proxy: proxy }, function(err, res, html) {
  ...
  request({ url: cssUrl, proxy: proxy }, function(err, res, css) {
  ...
  request({ url: jsUrl, proxy: proxy }, function(err, res, js) {
  ...
  if(this.isCordovaProject) {
    ...
    exec('cordova plugin add org.apache.cordova.device && ' +
         ...
         function(err, stdout, stderr) {
            if(err) {
              this.ionic.fail('Unable to add plugins. Perhaps your version of Cordova is too old. Try updating (npm install -g cordova), removing this project folder, and trying again.');
              q.reject(stderr);
            } else {
              q.resolve(stdout);
            }
