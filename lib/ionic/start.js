...
IonicStartTask.prototype._writeConfig = function(ionic) {
  console.log('Writing ionic.config', this.targetPath);
  var err = fs.writeFileSync(this.targetPath + '/' + ionic.IONIC_CONFIG, JSON.stringify({name: this.appName}, null, 2));
  if(err) {
    process.stderr.write('Error writing ' + this.targetPath + '/' + ionic.IONIC_CONFIG + ': ' + err + '\n');
    return;
  }
};
