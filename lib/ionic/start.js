  ...
  wrapperPromise.then(function() {
    ...
    Ionic.fetchRepo(self.targetPath, repoName, repoUrl).then(function(repoFolderName) {
      ...
      //Ionic.spawnPromise('npm', ['install', '--quiet']).then(function() {
        ...
        console.log('Initializing cordova project loco.'.info.bold);
        fs.mkdirSync(self.targetPath + '/.cordova');
        exec('echo "{}" >> .cordova/config.json');
        exec('cordova plugin add org.apache.cordova.device && ' + 
             'cordova plugin add org.apache.cordova.console && ' +
             'cordova plugin add org.apache.cordova.statusbar', function(err, stdout, stderr) {
          if(err) {
            pluginQ.reject(stderr);
          } else {
...
IonicStartTask.prototype._printQuickHelp = function(projectDirectory) {
  ...
  console.log('\nSome quick tips:');
  console.log('\n * cd into your project directory before running any of the following commands', ('(cd ' + projectDirectory + ')').info.bold);
  console.log('\n * Make sure to add a platform (ios or Android):', 'ionic platform add android [ios]'.info.bold);
