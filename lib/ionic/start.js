...
IonicStartTask.prototype.run = function(ionic) {
  ...
  this.isCordovaProject = (argv.cordova !== false && !argv.n);
...
IonicStartTask.prototype._fetchWrapper = function() {
  ...
  Ionic.fetchRepo(self.targetPath, repoName, repoUrl).then(function(repoFolderName) {
    ...
    return Ionic.fail('Error: Unable to fetch wrapper repo: ' + err);
  });
...
IonicStartTask.prototype._updateConfigXml = function() {
  ...
  try {
    ...
    parseString(configString, function (err, jsonConfig) {
      if(err) {
        return Ionic.fail('Error parsing config.xml: ' + err);
      }
      ...
      fs.writeFile(configXmlPath, configString, function(err) {
        if(err) {
          return Ionic.fail('Error saving config.xml file: ' + err);
        }
      });
    });

  } catch(e) {
    return Ionic.fail('Error updating config.xml file: ' + e);
  }
};
  ...
  wrapperPromise.then(function() {
    ...
    Ionic.fetchRepo(self.targetPath, repoName, repoUrl).then(function(repoFolderName) {
      ...
      pluginQ.promise.then(function() {
        ...
        return Ionic.fail('Unable to add plugins. Perhaps your version of Cordova is too old. Try updating (npm install -g cordova), removing this project folder, and trying again.');
      });
    ...
    return Ionic.fail( ('Unable to grab wrapper project: ' + err).error.bold );
