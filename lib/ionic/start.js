...
IonicTask.prototype.initCordova = function() {
  ...
  if(this.isCordovaProject) {
    ...
    //Adding in hooks so that when they are added, they are included in package.json (if not already)
    var cordovaTaskMod = require('./cordova').IonicTask;
    var cordovaTask = new cordovaTaskMod();
    cordovaTask.addHooks();
