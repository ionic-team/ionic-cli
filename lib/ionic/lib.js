  ...
  try {
    ...
    writeStream.on('close', function() {
      try{
        ...
        self.updateBowerFile(newVersion);
...
IonicLibTask.prototype.updateBowerFile = function(newVersion) {
  try {
    var bowerFilePath = path.resolve('bower.json');
    var bowerData;

    if( fs.existsSync(bowerFilePath) ) {
      bowerData = require(bowerFilePath);
    } else {
      bowerData = {
        "name": "HelloIonic",
        "private": "true"
    ...
    console.log( ('Bower file error: ' + e).error.bold );
  }
};
