...
IonicLibTask.prototype.getVersionData = function(version) {
  var q = Q.defer();
  var self = this;

  var url = self.codeHost;
  if(version == 'latest') {
    url += '/latest.json';
  } else {
...
IonicLibTask.prototype.downloadZip = function(version) {
  var self = this;
...
IonicLibTask.prototype.updateFiles = function(version) {
  var self = this;
    ...
    function onEntry(entry) {
      ...
      if(entry.type == 'Directory') {
        ...
        writeStream.on('error', function(err) {
          console.log( ('Error writing, ' + libEntryPath + ': ' + err).bold.red );
        });
...
IonicLibTask.prototype.writeVersionData = function() {
  try {
    var versionData = {
    ...
    var file = fs.createWriteStream( path.resolve('www/lib/ionic/version.json') );
    file.write( JSON.stringify(versionData, null, 2) );
    file.close();
  } catch(e) {
