...
function run(ionic, argv) {
  ...
  if (!fs.existsSync(path.resolve('www'))) {
    return fail('"www" directory cannot be found. Make sure the working directory ' +
                      'is at the top level of an Ionic project.', 'lib');
  }

  // Gather version data from version.json or bower.json files
  var data = loadVersionData(data);
...
function loadVersionData(data) {
  ...
  try {
    local = require(data.versionFilePath);
  } catch (e) {
...
function printLibVersions(data) {
  ...
  log.info('Local Ionic version: '.bold.green + data.local.version + '  (' + data.versionFilePath + ')');

  getVersionData('latest').then(function(versionData) {
    log.info('Latest Ionic version: '.bold.green + versionData.version_number +
...
function getLatest(data) {
  ...
  getVersionData(version).then(function(versionData) {
    if (version === 'latest') {
      log.info('Latest version: '.bold.green + versionData.version_number +
               '  (released ' + versionData.release_date + ')');
    } else {
      log.info('Version: '.bold.green + versionData.version_number +
               '  (released ' + versionData.release_date + ')');
    }
  ...
  try {
    ...
    writeStream.on('close', function() {
      ...
      try {
        ...
        log.info('Ionic version updated to: '.bold.green + data.versionData.version_number.bold);
