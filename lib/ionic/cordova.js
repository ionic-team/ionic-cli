...
CordovaTask.prototype.run = function(ionic, argv) {
  this.ionic = ionic;
  ...
  try {
    ...
    if (argv._.indexOf('run') !== -1 || argv._.indexOf('build') !== -1 || argv._.indexOf('emulate') !== -1) {
      ...
      var platformInstalled = self.checkIfPlatformInstalled(runPlatform);
      var pluginsInstalled = self.checkForPlugins();

      if (!platformInstalled) {
        var notInstalledMsg = [
          '• You\'re trying to build for ',
          runPlatform,
          ', but don\'t have the platform installed yet.'].join('').yellow;
...
CordovaTask.prototype.runCordova = function(cmdName, argv) {
...
CordovaTask.prototype.setupLiveReload = function(argv) {
  var d = Q.defer();
...
CordovaTask.prototype.checkIfPlatformInstalled = function checkIfPlatformInstalled(platform) {
...
CordovaTask.prototype.checkForPlugins = function checkForPlugins() {
