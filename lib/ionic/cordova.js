    ...
    parseString(configString, function (err, jsonConfig) {
      ...
      if(options.devServer) {
        ...
        if(jsonConfig.widget.content[0].$.src !== options.devServer) {
          ...
          console.log( ('Update config.xml for live reload: content[src="' + options.devServer + '"]').yellow );
          madeChange = true;
        }
        ...
        if( jsonConfig.widget.content[0].$['original-src'] ) {
          ...
          console.log( ('Reset config.xml after live reload build, return to content[src="' + jsonConfig.widget.content[0].$.src + '"]').yellow );
