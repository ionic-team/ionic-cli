...
IonicTask.prototype.addIonicIcons = function(platform) {

  // console.log('generate:', generate)
  // console.log('settings:', settings, '\n\n\n\n\n\n\n')

  //If the user already has the resources folder set up, 
  //we shouldnt need to do much here.
  if(fs.existsSync(path.join('./', 'resources'))) {
    console.log('Resources folder exists - no default resources shall be added'.red.bold);
    return;
  }

  //First we check to see if platforms exist in config.xml
  //If they do - let's not over write them.
  //If they dont - we need to go through the settings and place them there
  //then write config.xml file

  console.log('Adding icons for platform:'.blue.bold, platform.green)

  generate.getConfigData()
  .then(function(data) {
    if(!data.widget.platform || (data.widget.platform.length == 1 && data.widget.platform[0]['$'].name != platform)) {
      copyIconFilesIntoResources(platform)
      addPlatformIcons(platform, data)
    } else {
      console.log('Platform exists. Doing nothing'.red.bold);
    }
...
function copyIconFilesIntoResources(platform) {
  ...
  Ionic.fetchArchive(unzipPath, ionicResourcesUrl)
  .then(function() {
...
function addPlatformIcons(platform, configData) {
  var platElem = { '$': { name: platform }, icon: [] };
  if(!configData.widget.platform) {
    configData.widget.platform = [];
  }
  configData.widget.platform.push(platElem);
  settings.ResPlatforms[platform].icon.images.forEach(function(image) {
