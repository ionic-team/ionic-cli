...
var IonicSetupTask = function() {};

IonicSetupTask.prototype = new IonicTask();

IonicSetupTask.prototype.run = function(ionic) {
  if( argv._.length < 2 ) {
...
IonicSetupTask.prototype.sassSetup = function(ionic) {
  this.npmInstall().then(function(){
    ...
    try {
      ...
      console.log(' * Run'.yellow.bold, 'ionic serve'.bold, 'to compile to CSS and start a local dev server'.yellow.bold);
      console.log('');
...
IonicSetupTask.prototype.npmInstall = function() {
  var q = Q.defer();
...
exports.IonicSetupTask = IonicSetupTask;
