...
Ionic = {
  ...
  version: function() {
    ...
    process.stderr.write('Latest npm version: ');
    ...
    try {
      ...
      child.stdout.on('data', function(npmVersion) {
        ...
        if(npmVersion.trim() != settings.version.trim()) {
          process.stderr.write('Use ' + 'npm update -g ionic'.bold + ' to update to the latest version\n\n');
        }
      ...
      child.stderr.on('data', function(data) {
        ...
        process.stderr.write('Error loading current npm version: ' + data);
      });
      child.on('error', function(err) {
        process.stderr.clearLine();
        process.stderr.cursorTo(0);
        process.stderr.write('Error loading current npm version: ' + err);
      });
    } catch(e) {
      console.log(e)
    }
