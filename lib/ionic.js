...
Ionic = {
  ...
  fetchRepo: function(targetPath, repoName, repoUrl) {
    ...
    if(repoName.indexOf('/') === -1){
        repoFolderName = repoName + '-master'
    } else {
        repoFolderName = repoName.split("/")[1] + '-master'
    }
    console.log('\nDOWNLOADING:'.info.bold, repoUrl);

    var tmpFolder = os.tmpdir();
    //repoName.replace is to avoid having an extra level of depth in path which isn't created when the repoName has a /.
    var tempZipFilePath = path.join(tmpFolder, repoName.replace("/", "-") + new Date().getTime() + '.zip');
