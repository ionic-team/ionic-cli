  ...
  describe('lib/ssh-config', () => {
    ...
    describe('ensureHostAndKeyPath', () => {
      ...
      it('should stringify with config1 file', async () => {
        const config1 = await readFile(path.resolve(__dirname, 'fixtures/ssh-config/config1'), { encoding: 'utf8' });
      ...
      it('should stringify with config2 file', async () => {
        const config2 = await readFile(path.resolve(__dirname, 'fixtures/ssh-config/config2'), { encoding: 'utf8' });
      ...
      it('should stringify with config6 file', async () => {
        const config6 = await readFile(path.resolve(__dirname, 'fixtures/ssh-config/config6'), { encoding: 'utf8' });
