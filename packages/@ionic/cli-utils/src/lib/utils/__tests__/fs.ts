...
describe('@ionic/cli-utils', () => {
  ...
  describe('findBaseDirectory', () => {
    ...
    const cliUtilsFs = require('../fs');

    it('should get undefined with empty input', async () => {
      const result = await cliUtilsFs.findBaseDirectory('', '');
      expect(result).toEqual(undefined);
    });

    it('should return undefined if marker file not found', async () => {
      jest.spyOn(cliUtilsFs, 'fsReadDir').mockImplementation(() => Promise.resolve(['bar']));
      const result = await cliUtilsFs.findBaseDirectory('/some/dir', 'foo');
      expect(result).toEqual(undefined);
    });

    it('should return path when marker file found in cwd', async () => {
      jest.spyOn(cliUtilsFs, 'fsReadDir').mockImplementation(() => Promise.resolve(['foo']));
      const result = await cliUtilsFs.findBaseDirectory('/some/dir', 'foo');
      expect(result).toEqual('/some/dir');
    });

    it('should return path when marker file found in one directory back', async () => {
      jest.spyOn(cliUtilsFs, 'fsReadDir')
    ...
    it('should return path when marker file found in really nested path', async () => {
      jest.spyOn(cliUtilsFs, 'fsReadDir')
        ...
        .mockImplementationOnce(() => Promise.resolve(['dir', 'foo']));
      const result = await cliUtilsFs.findBaseDirectory('/some/dir/that/is/really/nested', 'foo');
      expect(result).toEqual('/some');
    });
    ...
    describe('windows', () => {
      ...
      it('should return undefined if marker file not found', async () => {
        jest.spyOn(cliUtilsFs, 'fsReadDir').mockImplementation(() => Promise.resolve(['bar']));
        const result = await cliUtilsFs.findBaseDirectory('C:\\some\\dir', 'foo');
        expect(result).toEqual(undefined);
      });
      ...
      it('should return path when marker file found in really nested path', async () => {
        jest.spyOn(cliUtilsFs, 'fsReadDir')
          ...
          .mockImplementationOnce(() => Promise.resolve(['that']))
          .mockImplementationOnce(() => Promise.resolve(['dir', 'foo']));
        const result = await cliUtilsFs.findBaseDirectory('C:\\some\\dir\\\\that\\is\\really\\nested', 'foo');
        expect(result).toEqual('C:\\some');
      });
