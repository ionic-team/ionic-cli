...
import { copyDirectory, fsMkdir, fsReadFile, fsUnlink, fsWriteFile, pathExists, removeDirectory } from '@ionic/cli-framework/utils/fs';
...
${chalk.cyan('[1]')}: ${chalk.bold('https://ionicframework.com/docs/cli/starters.html')}
      ...
      options: [
        ...
        {
          name: 'structure',
          summary: `To stratify the folder structure. page Components move ${chalk.green('app/')} to ${chalk.green('app/pages/')} (e.g. ${chalk.red('type angular only')})`,
          type: Boolean,
          aliases: ['s'],
        },
        {
  ...
  async run(inputs: CommandLineInputs, options: CommandLineOptions, runinfo: CommandInstanceInfo): Promise<void> {
    ...
    // If type is angular and add option structure, structure changed.
    // $ ionic start --type=angular --structure=true
    if (options.structure && this.schema.type === 'angular') {
      await this.rewriteStructureTypeAngular(projectDir);
    }
  ...
  async rewriteStructureTypeAngular(projectDir: string) {
    const appHome = projectDir + '/src/app';
    return new Promise(resolve => {
      fsReadFile(appHome + '/app-routing.module.ts', { encoding: 'UTF-8' })
        .then(module => {
          const loadChildren = module.match(/loadChildren: \'(.+)\#/g);
          if (loadChildren !== null) {
            fsMkdir(appHome + '/pages', 0o777)
              .then(comp1 => {
                loadChildren.forEach(filepath => {
                  const folder = filepath.match(/loadChildren: \'\.\/(.+)\//);
                  if (folder !== null) {
                    copyDirectory(appHome + '/' + folder[1], appHome + '/pages/' + folder[1]).then(
                      comp2 => removeDirectory(appHome + '/' + folder[1])
                    ).catch();
                  }
                });
                const newModule = module.replace(/loadChildren: \'\./g, 'loadChildren: \'./pages');
                fsWriteFile(appHome + '/app-routing.module.ts', newModule, { encoding: 'UTF-8' }).then().catch();
              }).catch();
          }
          resolve();
        })
