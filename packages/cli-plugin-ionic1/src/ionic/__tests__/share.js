...
describe('share command', function() {
  ...
  describe('run function', function() {
    ...
    it('should fail if project.load fails', function() {
      ...
      spyOn(Project, 'load').and.callFake(function() {
        throw error;
      });
      spyOn(appLibUtils, 'fail');
    ...
    it('should fail if the project does not have an app_id', function() {
      ...
      spyOn(Project, 'load').and.returnValue(projectSpy);
      projectSpy.get.and.returnValue('');
      spyOn(appLibUtils, 'fail');
    ...
    it('should fail if the email provided does not have an @ sign within it', function() {
      ...
      var projectSpy = jasmine.createSpyObj('promiseExec', ['get']);
      spyOn(Project, 'load').and.returnValue(projectSpy);
      projectSpy.get.and.returnValue('123');
      spyOn(appLibUtils, 'fail');
    ...
    it('should fail if the login.retrieveLogin returns a rejected promise', function(done) {
      ...
      spyOn(Project, 'load').and.returnValue(projectSpy);
      projectSpy.get.and.returnValue('123');
      spyOn(Login, 'retrieveLogin').and.returnValue(Q.reject('error occurred'));
      spyOn(appLibUtils, 'fail').and.returnValue(Q(true)); // TODO: hack for now
    ...
    it('should not ask a user to login if a previous session is found', function(done) {
      ...
      spyOn(log, 'info');
      spyOn(process, 'cwd').and.returnValue('/stuff/things');
      var projectSpy = jasmine.createSpyObj('promiseExec', ['get']);
      spyOn(Project, 'load').and.returnValue(projectSpy);
      projectSpy.get.and.returnValue('123');
      spyOn(Login, 'retrieveLogin').and.returnValue(Q({ jar: 'contents' }));
      spyOn(LoginTask, 'login');
      spyOn(Share, 'shareApp').and.returnValue(Q(true));
    ...
    it('should ask a user to login if a previous session is not found', function(done) {
      ...
      spyOn(Project, 'load').and.returnValue(projectSpy);
      projectSpy.get.and.returnValue('123');
      spyOn(Login, 'retrieveLogin').and.returnValue(Q(false));
      spyOn(LoginTask, 'login').and.returnValue(Q({ jar: 'contents' }));
      spyOn(Share, 'shareApp').and.returnValue(Q(true));
